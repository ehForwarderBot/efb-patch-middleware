    ...
    def update_group_info(self, update: Update, context: CallbackContext):
        ...
        try:
            ...
            if isinstance(chat, GroupChat):
                ...
                if chat.members:
                    chat_members = list(filter(lambda member: member.id not in ('__self__', '__system__'), chat.members))
                    # TRANSLATORS: Separator between group members in a Telegram group description generated by /update_info
                    members = self._(", ").join(i.long_name for i in chat_members)
                    if desc:
                        desc += "\n"
                    desc += self.ngettext("{count} group member: {list}",
                                        "{count} group members: {list}",
                                        len(chat_members)).format(
                        count=len(chat_members), list=members
                    )
                if desc:
                    try:
